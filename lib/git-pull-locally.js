...
const tempdir = path.join(os.tmpdir(), repoPathLocal);
...
git.status(function _gitStatusHandler(err, status) {
  ...
  if (!isGitRepo) {
    ...
    // 4b. if it is existing repo, check if remote is correct, if not throw error
    // git remote -v
    git.getRemotes(true, function _gitRemotesHandler(err, remotesList) {
      if (err) throw `Error checking list of remotes: ${err}.`;
      ...
      if (
        ...
        throw `Existing local repository remotes aren't correct:
...
Remove ${tempdir} directory and run again.`;
        ...
        git.fetch('origin', 'master', (err, res) => {
          if (err) throw `Error when fetching: ${err}.`;
          console.log('* 6a. Fetch complete:', res);
        })
          .reset(['-hard FETCH_HEAD'], (err, res) => {
            if (err) throw `Error when resetting: ${err}.`;
            console.log('* 6b. Reset complete:', res);
          })
          .clean('f', ['-d'], (err, res) => {
            if (err) throw `Error when cleaning: ${err}.`;
            console.log('* 6c. Cleaning complete:', res);
          })
          .then(v => console.log('* 6d. Finished pulling:', v));
      }
    });
  }
});
